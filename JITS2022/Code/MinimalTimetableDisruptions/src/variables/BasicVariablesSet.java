...
public class BasicVariablesSet extends RouteVariablesSet {
	...
	public HashMap<String, Double>[][] initialSolution = null;
...
public void setWarmStart() throws IloException {
	    ...
	    for (int b = 0; b < instance.b; b++) {
			for (int k = 1; k < instance.paths[b].length; k++) {
				if (initialSolution[b] != null) {
					...
					HashMap<String, Double> varVals = initialSolution[b][k];
		...
		//cplex.addMIPStart(startVarXB, startValXB);
		cplex.addMIPStart(startVarAT, startValAT);
		...
		if (mustFix) {
			System.out.println("Fixing  route variables");
			//System.out.println("Printing them...");
			for (int b = 0; b < instance.b; b++) {
				for (int k = 1; k < instance.paths[b].length; k++) {
					if (initialSolution[b] != null) {
						HashMap<String, Double> varVals = initialSolution[b][k];
						arrivalTime[b][k].setLB(varVals.get("arrivalTime"));
						arrivalTime[b][k].setUB(varVals.get("arrivalTime"));
						ct[b][k].setLB(varVals.get("ct"));
						ct[b][k].setUB(varVals.get("ct"));
					}
				}
			}
		}
