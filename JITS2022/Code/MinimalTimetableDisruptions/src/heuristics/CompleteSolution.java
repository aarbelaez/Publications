...
public class CompleteSolution {
	...
	public boolean areAllOpenStationsCharging() {
		...
		for (HashMap<Integer, OpenStation> busPath: openStationsStructure.openStopsPerBus) {
			for (Integer os: busPath.keySet()) {
				int b = busPath.get(os).bus;
				int k = busPath.get(os).stop;
	...
	public void writeResults(double elapsedTime, int numLocalMins) {
		normalVars.writeResults(elapsedTime, numLocalMins);
	}
	...
	public boolean checkLinkedListConsistencyOpenStopsPerBus() {
		return openStationsStructure.checkLinkedListConsistencyOpenStopsPerBus();
	}
	*/
	...
	public boolean checkForNegativeSlacks() {
		return openStationsStructure.checkForNegativeSlacks();
	}
