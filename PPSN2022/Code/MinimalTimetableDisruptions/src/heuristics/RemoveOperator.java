...
public class RemoveOperator {
	...
	private final static Logger LOGGER = Logger.getLogger(RemoveOperator.class.getName());
	...
	public boolean isFeasible2(int b, int k) {
		...
		while (openStations.hasNext()) {
			selectedStation = openStations.next();
			if (selectedStation.bus == b && selectedStation.stop == k) {
	 ...
	 * @return
	 */
	public boolean removeAllBuses(int i, int seed, int iter) {
				...
				//for (int k = 0; k < numBusesInI; k++) {
 				for (OpenStation selectedStop: nopenStopsInI) {
					...
					*/
					...
					if (this.isFeasible2(selectedStop.bus, selectedStop.stop)) {
						this.removeStation();
					} else {
						return false;
