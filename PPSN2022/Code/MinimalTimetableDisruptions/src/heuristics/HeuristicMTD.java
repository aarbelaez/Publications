	...
	public void runHeuristic(int execTime) throws FileNotFoundException {
		...
		for (int i = 0; i < maxIters; i++) {
			...
			if (stationIds.isEmpty()) {
				...
				if (prevObj <= obj && i > 0) {				
					...
					*/
					...
					int addingStations = (int) Math.round(obj*0.4);
			...
			//if (removeOperator.checkAllBuses(selectedStation, seed)) {
				...
				if (!removeOperator.removeAllBuses(selectedStation, seed, i)) {
			...
			*/
			...
			/*
			if (i > 2500 && !currentSolution.checkFeasibility(instance, "partial.log")) {
		...
		*/
		...
		if (bestCurrentSolution != null) {
			...
			overallBestCurrentSolution.writeResults(maxExecTime/1000);
