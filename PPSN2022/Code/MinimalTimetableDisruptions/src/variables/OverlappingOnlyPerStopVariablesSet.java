...
public class OverlappingOnlyPerStopVariablesSet extends VariablesSet {
	 ...
	 */
	public Map<Integer, Map<Integer, Map<Integer, Map<Integer, IloIntVar>>>> Z;
	/**
	...
	public void defineVariables() throws IloException {
		// buses b and d are using the same charging station
		Z = new HashMap<Integer, Map<Integer, Map<Integer, Map<Integer, IloIntVar>>>>();
		...
		for (int b = 0; b < instance.b; b++) {
			Z.put(b, new HashMap<Integer, Map<Integer, Map<Integer, IloIntVar>>>());
			...
			for (int d = 0; d < (breakBusSymmetries ? b : instance.b); d++) {		
				...
				if (b != d) {
					...
					Z.get(b).put(d, new HashMap<Integer, Map<Integer, IloIntVar>>());
					...
					for (int k = 0; k < instance.paths[b].length; k++) {
						Z.get(b).get(d).put(k, new HashMap<Integer, IloIntVar>());
						...
						for (int m = 0; m < instance.paths[d].length; m++) {
							...
							if (i == j  && 
								...
								Z.get(b).get(d).get(k).put(m, cplex.boolVar());
