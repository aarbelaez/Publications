...
void Ubcsat::ActivateTriggers(char *sTriggers) {
  ParseItemList(&listTriggers,sTriggers,&Ubcsat::ActivateTriggerID);
}

void Ubcsat::DeActivateTriggers(char *sTriggers) {
  ParseItemList(&listTriggers,sTriggers,&Ubcsat::DeActivateTriggerID);
}


void Ubcsat::ActivateColumns(char *sColumns) {
  ParseItemList(&listColumns,sColumns,&Ubcsat::ActivateColumnID);
}

void Ubcsat::ActivateStats(char *sStats) {
  ParseItemList(&listStats,sStats,&Ubcsat::ActivateStatID);
}

void Ubcsat::ActivateAlgorithmTriggers() {
  Ubcsat::ActivateTriggers(pActiveAlgorithm->sDataTriggers);
  Ubcsat::ActivateTriggers(pActiveAlgorithm->sHeuristicTriggers);
}

void Ubcsat::ActivateColumnID(UINT32 iColID, const char *sItem) {
...
void Ubcsat::ActivateStatID(UINT32 iStatID, const char *sItem) {
  ...
  if (pStat->bActive == FALSE) {
    ...
    pPos = const_cast<char *>(strchr(sItem,'['));
    if (pPos) {
      pPos++;
      pPos2 = strchr(pPos,']');
      if (pPos2) {
    ...
    if (pStat->bCustomField==FALSE) {
      ...
      if (pStat->iStatFlags & STATCODE_RAMMASK) {
        ParseItemList(&listColumns,pStat->sDataColumn,&Ubcsat::AddAllocateRAMColumnID);
      }
...
void Ubcsat::ActivateTriggerID(UINT32 iFxnID, const char *sItem) {
  ...
  ParseItemList(&listTriggers,pTrigger->sDependencyList,&Ubcsat::ActivateTriggerID);
  ParseItemList(&listTriggers,pTrigger->sDeactivateList,&Ubcsat::DeActivateTriggerID);
...
void Ubcsat::ActivateDynamicParms() {
...
void Ubcsat::AddAllocateRAMColumnID(UINT32 j, const char *sItem) {
  REPORTCOL *pCol;
  NOREF(sItem);
...
void Ubcsat::AddDynamicParm(void *pTarget, enum CDATATYPE eDataType, UINT32 *pBase, FLOAT fFactor) {
...
void Ubcsat::AddColumnComposite(const char *sID, 
                        const char *sList)
{
  AddContainerItem(&listColumns,sID,sList);
}

void Ubcsat::AddColumnFloat(const char *sID, 
...
void Ubcsat::AddColumnUInt(const char *sID, 
                  const char *sDescription, 
                  char *sHeader1,  
                  char *sHeader2,  
                  char *sHeader3, 
...
void Ubcsat::AddContainerItem(ITEMLIST *pList,const char *sID, const char *sList) {
...
void Ubcsat::AddItem(ITEMLIST *pList,const char *sID) {
  SetString(&pList->aItems[pList->iNumItems].sID,sID);
...
void Ubcsat::AddOutputColumnID(UINT32 j, const char *sItem) {
...
void Ubcsat::AddParmBool(ALGPARMLIST *pParmList, 
                  const char *sSwitch, 
...
void Ubcsat::AddParmFloat(ALGPARMLIST *pParmList, 
                  const char *sSwitch, 
                  const char *sTerseDescription, 
                  const char *sVerboseDescription,
                  const char *sTriggers,
...
ALGPARM *Ubcsat::AddParmCommon(ALGPARMLIST *pParmList,
                  const char *sSwitch, 
                  const char *sTerseDescription, 
                  const char *sVerboseDescription,
                  const char *sTriggers)
...
void Ubcsat::AddParmUInt(ALGPARMLIST *pParmList, 
                  const char *sSwitch, 
                  const char *sTerseDescription, 
                  const char *sVerboseDescription,
                  const char *sTriggers,
...
void Ubcsat::AddParmProbability(ALGPARMLIST *pParmList,
                  const char *sSwitch, 
                  const char *sTerseDescription, 
                  const char *sVerboseDescription,
                  const char *sTriggers,
...
void Ubcsat::AddParmReport(ALGPARMLIST *pParmList,
                  const char *sSwitch, 
                  const char *sTerseDescription, 
                  const char *sVerboseDescription,
                  const char *sTriggers
...
void Ubcsat::AddParmSInt(ALGPARMLIST *pParmList, 
                  const char *sSwitch, 
                  const char *sTerseDescription, 
                  const char *sVerboseDescription,
                  const char *sTriggers,
...
void Ubcsat::AddParmString(ALGPARMLIST *pParmList, 
                  const char *sSwitch, 
                  const char *sTerseDescription, 
                  const char *sVerboseDescription,
                  const char *sTriggers,
...
void Ubcsat::AddReportParmCommon(REPORT *pRep, const char *sParmName) {
...
void Ubcsat::AddReportParmFloat(REPORT *pRep, const char *sParmName, FLOAT *pParmValFloat, FLOAT fDefault) {
...
void Ubcsat::AddReportParmString(REPORT *pRep, const char *sParmName, const char *pDefault) {
  pRep->aParmTypes[pRep->iNumParms] = PTypeString;
  SetString((char **) &pRep->aParameters[pRep->iNumParms],pDefault);
  AddReportParmCommon(pRep,sParmName);
}

void Ubcsat::AddReportParmUInt(REPORT *pRep, const char *sParmName, UINT32 *pParmValUInt, UINT32 iDefault) {
...
void Ubcsat::AddRTDColumnID(UINT32 j, const char *sItem) {
...
void Ubcsat::AddStatCol(const char *sID, 
...
void Ubcsat::AddContainerStat(const char *sID, 
                      const char *sList) {

  AddContainerItem(&listStats,sID,sList);
}

void Ubcsat::AddStatCustom(const char *sID, 
...
void Ubcsat::CalculateStats(FLOAT *fMean, FLOAT *fStddev, FLOAT *fCV, FLOAT fSum, FLOAT fSum2, UINT32 iCount) {
...
FLOAT Ubcsat::CorrelationCoeff(FLOAT fSumA, FLOAT fSumA2,FLOAT fSumB, FLOAT fSumB2, FLOAT fSumAB, UINT32 iCount) {
...
void Ubcsat::CheckInvalidParamters() {
...
void Ubcsat::CheckParamterFile(int iCommandLineCount,char **aCommandLineArgs) {
  int j;
  UINT32 k;
...
void Ubcsat::ClearActiveProcedures() {
  memset(aNumActiveProcedures,0,sizeof(UINT32) * NUMEVENTPOINTS);
}

void Ubcsat::CopyParameters(ALGORITHM *pDest, const char *sName, const char *sVar, BOOL bWeighted) {
  UINT32 j;
...
ALGORITHM *Ubcsat::CreateAlgorithm (const char *sName, const char *sVariant, BOOL bWeighted, 
                            const char *sDescription, 
                            const char *sAuthors,
...
void Ubcsat::CreateContainerTrigger(const char *sID, const char *sList) {
  AddContainerItem(&listTriggers,sID,sList);
}

REPORT *Ubcsat::CreateReport(const char *sID, const char *sDescription, const char *sVerboseDescription, const char *sOutputFile, const char *sTriggers) {
  REPORT *pRep;
...
void Ubcsat::CreateTrigger2(const char *sID, enum EVENTPOINT eEventPoint, FXNPTR pProcedure, char *sDependencyList, char *sDeactivateList) {
  TRIGGER *pNewTrig;
...
void Ubcsat::DeActivateTriggerID(UINT32 iFxnID, const char *sItem) {
...
ALGORITHM *Ubcsat::FindAlgorithm(const char *sFindName, const char *sFindVar, BOOL bFindWeighted) {
  UINT32 j;
  if (*sFindName != 0) {
    for (j=0;j<iNumAlg;j++) {
...
UINT32 Ubcsat::FindItem(ITEMLIST *pList,char *sID) {
  SINT32 j;
  char *pPos;
  char *pPos2;
  UINT32 iLen;
...
ALGPARM *Ubcsat::FindParm(ALGPARMLIST *pParmList, char *sSwitch) {
...
void Ubcsat::InheritDataTriggers(ALGORITHM *pDest, const char *sName, const char *sVar, BOOL bWeighted) {
  ALGORITHM *pSrc = FindAlgorithm(sName,sVar,bWeighted);
  if (pSrc==0) {
    ReportPrint1(pRepErr,"Unexpected Error: Can't find algorithm %s\n",(char *) sName);
    AbnormalExit();
...
BOOL Ubcsat::IsLocalMinimum(BOOL bUseWeighted) {
  UINT32 j,k;
  UINT32 iNumOcc;
  UINT32 *pClause;
  LITTYPE litCur;
...
UINT32 Ubcsat::MatchParameter(char *sSwitch,char *sParm) {
...
void Ubcsat::ParseAllParameters(int argc, char *argv[]) {
...
void Ubcsat::ParseItemList(ITEMLIST *pList, char *sItems, CALLBACKPTR ItemFunction) {
  ...
  if (pList->aItems[j].bContainer) {
    ...
    (*this.*ItemFunction)((UINT32) j,sItems);
  }
}

void Ubcsat::ParseParameters(ALGPARMLIST *pParmList) {
  ...
  while (iCurParm < iNumTotalParms) {
    ...
    if (pParm) {
      ...
      ParseItemList(&listTriggers,pParm->sTriggers,&Ubcsat::ActivateTriggerID);
...
void Ubcsat::PrintAlgParmSettings(REPORT *pRep, ALGPARMLIST *pParmList) {
...
void Ubcsat::SetAlgorithmDefaultReports() {
...
void Ubcsat::SetDefaultParms(ALGPARMLIST *pParmList) {
  UINT32 j;
  ALGPARM *pParm;
...
UINT32 Ubcsat::SetColStatFlags (char *sStatsParms) {
...
VARSTATE Ubcsat::NewCopyVarState(VARSTATE vsCopy) {
  VARSTATE vsNew;
  UINT32 j;
  vsNew = (BYTE*)AllocateRAM(iVARSTATELen * sizeof(BYTE));
  for (j=0;j<iVARSTATELen;j++) {
...
void Ubcsat::SetCurVarState(VARSTATE vsIn) {
  UINT32 j;
...
BOOL Ubcsat::SetCurVarStateString(VARSTATE vsIn, char *sVarState) {
  UINT32 j;
...
void Ubcsat::SetArrayFromVarState(UINT32 *aOut, VARSTATE vsIn) {
  UINT32 j;
...
UINT32 Ubcsat::HammingDistVarState(VARSTATE vsA, VARSTATE vsB) {
  UINT32 j;
  UINT32 iDist = 0;
  BYTE *pA = vsA;
  BYTE *pB = vsB;
...
BOOL Ubcsat::IsVarStateEqual(VARSTATE vsA, VARSTATE vsB) {
  UINT32 j;
  BYTE *pA = vsA;
  BYTE *pB = vsB;
  if ((vsA == NULL)||(vsB == NULL)) {
...
BOOL Ubcsat::IsVarStateInList(VARSTATELIST *vsList, VARSTATE vsIn) {
...
UINT32 Ubcsat::MinHammingVarStateList(VARSTATELIST *vsList, VARSTATE vsIn) {
  VARSTATELIST *vsNext = vsList;
...
void Ubcsat::AddToVarStateList(VARSTATELIST *vsList, VARSTATE vsAdd) {
  VARSTATELIST *pNext;
...
BOOL Ubcsat::AddUniqueToVarStateList(VARSTATELIST *vsList, VARSTATE vsAdd) {
