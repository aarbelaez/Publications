...
void Ubcsat::PostStepIRoTS() {
  ...
  if (bRestore) {
	...
	RunProceduresALEJO2(InitStateInfo);
	RunProceduresALEJO2(PostInit);
  }

}


void Ubcsat::CreateIRoTSBackup() {
  aIrotsBackup = (UINT32*)AllocateRAM((iNumVars+1)*sizeof(UINT32));
}

void Ubcsat::IRoTSBackup() {
  UINT32 j;
