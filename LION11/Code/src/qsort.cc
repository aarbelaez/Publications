...
#define Pivot(swapper,sz)			\
  if (last-first>PIVOT_THRESHOLD*sz) mid=pivot_big(first,mid,last,sz,compare,usat);\
  else {	\
    if (compare2(first,mid)<0) {			\
      if (compare2(mid,last)>0) {		\
        swapper(mid,last);			\
        if (compare2(first,mid)>0) swapper(first,mid);\
      }						\
    }						\
    else {					\
      if (compare2(mid,last)>0) swapper(first,last)\
      else {					\
        swapper(first,mid);			\
        if (compare2(mid,last)>0) swapper(mid,last);\
      }						\
    }						\
    first+=sz; last-=sz;			\
  }
...
#define Partition(swapper,sz) {			\
  int swapped=0;				\
  do {						\
    while (compare2(first,pivot)<0) first+=sz;	\
    while (compare2(pivot,last)<0) last-=sz;	\
    if (first<last) {				\
...
#define PreInsertion(swapper,limit,sz)		\
  ...
  while (last!=base) {				\
    if (compare2(first,last)>0) first=last;	\
    last-=sz; }					\
...
#define Insertion(swapper)			\
  ...
  for (first=((char*)base)+size;first!=last;first+=size) {	\
    ...
    for (test=first-size;compare2(test,first)>0;test-=size) ;	\
    test+=size;					\
    if (test!=first) {				\
...
typedef int (Ubcsat::*ComparePTR)(const void *, const void *);

/*static char * pivot_big(char *first, char *mid, char *last, size_t size,
                        int Ubcsat::*compare(const void *, const void *), Ubcsat* usat) {*/
static char * pivot_big(char *first, char *mid, char *last, size_t size,
                        ComparePTR compare, Ubcsat* usat) {						
...
#endif
    m1 = compare2(a,b)<0 ?
           (compare2(b,c)<0 ? b : (compare2(a,c)<0 ? c : a))
         : (compare2(a,c)<0 ? a : (compare2(b,c)<0 ? c : b));
  }
...
#endif
  return compare2(m1,m2)<0 ?
           (compare2(m2,m3)<0 ? m2 : (compare2(m1,m3)<0 ? m3 : m1))
         : (compare2(m1,m3)<0 ? m1 : (compare2(m2,m3)<0 ? m3 : m2));
}
...
fprintf(stderr,"pivot=%d\n",*(int*)pivot);
  ...
  for (first=((char*)base)+WORD_BYTES;first!=last;first+=WORD_BYTES) {
    ...
    *(int*)pivot=*(int*)first;
    for (;compare2(pl,pivot)>0;pr=pl,--pl) {
      *pr=*pl; }
...
extern void qsortG(void *base, size_t nmemb, size_t size,
  ...
  if (((uintptr_t)base|size)&(WORD_BYTES-1))
    qsort_nonaligned(base,nmemb,size,compare,usat);
  else if (size!=WORD_BYTES)
    qsort_aligned(base,nmemb,size,compare,usat);
  else
    qsort_words(base,nmemb,compare,usat);
}
